        ...
        match instruction {
            ...
            Instruction::CreateFile { filename, contents } => {
                ...
                system.files.insert(
                    filename,
                    VirtualFile {
                        contents: VirtualFileContents::Bytes(contents_string),
                    },
                );
            }
...
fn run_tests() {
    ...
    for entry in fs::read_dir("./test-files").expect("couldn't read test-files") {
        ...
        source_path.push("source.rbxl");
        
        // Try .rbxl first, then .rbxm as fallback
        let source_file = if source_path.exists() {
            source_path
        } else {
            source_path.set_extension("rbxm");
            if !source_path.exists() {
                panic!("Could not find source.rbxl or source.rbxm in {:?}", path);
            }
            source_path
        };

        let source = fs::File::open(&source_file).expect("couldn't read source file");
        let source_reader = BufReader::new(source);

        let time = Instant::now();
        let tree = rbx_binary::from_reader_default(source_reader).expect("couldn't deserialize source file");
        info!(

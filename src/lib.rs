...
fn repr_instance<'a>(
    ...
    if !include_branch && has_scripts.get(&child.referent()) != Some(&true) {
        return None;
    }
    ...
    match child.class.as_str() {
                    ...
                    Instruction::CreateFile {
                        ...
                        contents: Cow::Owned(
                            serde_json::to_string_pretty(&MetaFile {
                                class_name: None,
                                properties: properties_from_instance(
                                    child,
        ...
        "Script" | "LocalScript" | "ModuleScript" => {
            ...
            let source_key: Ustr = ustr::ustr("Source");
            let source = match child.properties.get(&source_key).expect("no Source") {
            ...
            if child.children().is_empty() {
                ...
                let meta_contents = Cow::Owned(
                    serde_json::to_string_pretty(&MetaFile {
                        class_name: None,
                        properties: properties_from_instance(child, include_all_properties),
                        ignore_unknown_instances: true,
                    })
                ...
                let represented_children_count = child
                    .children()
                    .iter()
                    .filter(|id| include_branch || has_scripts.get(id) == Some(&true))
                    .count();
                ...
                match represented_children_count {
                    _ if represented_children_count == total_children_count => Some((
                        vec![
...
impl<'a, I: InstructionReader + ?Sized> TreeIterator<'a, I> {
    fn visit_instructions(&mut self, instance: &Instance, has_scripts: &HashMap<Ref, bool>) {
        for child_id in instance.children() {
            ...
            let child_inside_starter_gui = self.inside_starter_gui || child.class == "StarterGui";
            let include_this_branch = self.include_branch || child_inside_starter_gui;
            ...
            let (instructions_to_create_base, path) = if child.class == "StarterPlayer" {
                ...
                if include_this_branch || has_scripts.get(child_id) == Some(&true) {
                    instructions.push(Instruction::CreateFolder {
                        folder: folder_path.clone(),
                    });
                    ...
                    instructions.push(Instruction::AddToTree {
                        ...
                        partition: TreePartition {
                            ...
                            children: child
                                .children()
                                .iter()
                                .filter(|id| self.include_branch || has_scripts.get(id) == Some(&true))
                                .map(|child_id| {
            ...
            TreeIterator {
                ...
                path: &path,
                tree: self.tree,
                include_branch: include_this_branch,
                inside_starter_gui: child_inside_starter_gui,
            }
